...
public class MergeSort {
    ...
    private static void merge(Comparable[] a, Comparable[] aux, int lo, int mid, int hi, Comparator comp){
        ...
        for (int k = lo; k <= hi; k++){
            ...
            else if (less(aux[j], aux[i], comp)) a[k] = aux[j++];
            else a[k] = aux[i++];
        }
    ...
    private static void sort(Comparable[] a, Comparable[] aux, int lo, int hi, Comparator comp){
        if (hi <= lo) return;
        if (hi <= lo + CUTOFF - 1) InsertionSort.sort(a,lo,hi, comp);
        else {int mid = lo + (hi - lo) / 2;
            sort(a, aux, lo, mid, comp);
            sort(a, aux, mid+1, hi, comp);
            if (!less(a[mid+1],a[mid], comp)) return;
    ...
    private static boolean less(Comparable v, Comparable w, Comparator comp){ return comp.compare(v, w)<0; }
}